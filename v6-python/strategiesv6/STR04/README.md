...
```
python -m strategiesv6 --strategy=str04
```

Dispatches to `loader.py:main()` via [strategiesv6/\_\_main\_\_.py](../__main__.py).
Running `loader.py` directly also works: `python -m strategiesv6.STR04.loader`.

## Config / secrets

Uses `premarketv6.config.load_databento().api_key` -- same key already used by
