#!/usr/bin/env python3
"""CME Globex MDP 3.0 (dataset id ``GLBX.MDP3``) ``db.Live()``: ``SymbolMappingMsg`` (``stype_in_symbol`` → ``stype_out_symbol``, ``instrument_id``). API key: ``__________v3/config.ini`` ``api_key_es`` (or ``DATABENTO_API_KEY_ES``).
...
No positional and no ``--symbols-file``: built-in ES weekly roots (see ``_ES_PARENT_ROOTS_CSV``): bare tokens become ``ROOT.OPT``; tokens that already contain ``.`` (e.g. ``ES.FUT``, ``ES.V.0``) are used as-is. ``stype_in=parent``, one Live subscribe per symbol, rows appended to ``YYYYMMDD/glbx_mdp3.csv`` after each.
...
def main() -> int:
    ...
    p.add_argument(
        "--csv",
        dest="csv_out",
        default=str(_default_csv_out("glbx_mdp3.csv")),
        metavar="PATH",
        help="write symbol_mappings to this CSV (default: YYYYMMDD/glbx_mdp3.csv under this script dir; empty to skip)",
    )
    p.add_argument(
        "--historical-fallback",
        action="store_true",
    ...
    try:
        from v3_config import get_api_key_es
