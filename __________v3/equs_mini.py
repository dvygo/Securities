#!/usr/bin/env python3
"""EQUS.MINI ``db.Live()``: ``SymbolMappingMsg`` (``stype_in_symbol`` → ``stype_out_symbol``, ``instrument_id``). Key: ``__________v3/config.ini`` ``api_key`` (or ``DATABENTO_API_KEY``).

One ticker (positional): subscribe with ``stype_in=raw_symbol``, stop on first mapping.

No positional: load tickers from ``underlyings.csv`` (same dir as this script), ``stype_in=raw_symbol``. Override list via ``--symbols-file``. Each symbol gets its own Live session; rows append to ``YYYYMMDD/equs_mini.csv``.
"""
    ...
    if csv_p.exists() and csv_p.stat().st_size > 0:
        ...
        if first != expected:
            print(
                ...
                f"expected {len(expected)}). Delete the file before appending Live symbology.",
                file=sys.stderr,
            )
            raise SystemExit(2)
...
def main() -> int:
    p = argparse.ArgumentParser(
        description=f"{DATASET} Live SymbolMappingMsg (default: underlyings.csv, stype_in=raw_symbol; or one ticker positional).",
    )
    p.add_argument(
        "raw",
        nargs="?",
    ...
    p.add_argument(
        ...
        help="underlyings list (one ticker per line; default: underlyings.csv next to this script)",
    )
    ...
    p.add_argument(
        "--csv",
        dest="csv_out",
        default=str(_default_csv_out("equs_mini.csv")),
        metavar="PATH",
        help="append symbol_mappings (default: YYYYMMDD/equs_mini.csv; empty to skip)",
    )
    p.add_argument(
        "--historical-fallback",
        action="store_true",
