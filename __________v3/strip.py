#!/usr/bin/env python3
"""Filter ``opra_pillar_unstripped.csv`` → ``opra_pillar.csv`` (near-term OPRA weeklies).
...
Default paths (under this script's directory):
  ``YYYYMMDD/opra_pillar_unstripped.csv`` → ``YYYYMMDD/opra_pillar.csv``
...
def _default_csv_paths(as_of: date | None = None) -> tuple[Path, Path]:
    ...
    day_dir = _V3_DIR / d
    return day_dir / "opra_pillar_unstripped.csv", day_dir / "opra_pillar.csv"
...
def main(argv: list[str] | None = None) -> int:
    default_in, default_out = _default_csv_paths()
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--input", type=Path, default=default_in, help="Unstripped symbology CSV")
    p.add_argument("--output", type=Path, default=default_out, help="Stripped output CSV")
    p.add_argument(
        "--as-of",
        type=str,
        default=None,
    ...
    if not in_path.is_absolute():
        in_path = (_V3_DIR / in_path).resolve()
    ...
    if not out_path.is_absolute():
        out_path = (_V3_DIR / out_path).resolve()
