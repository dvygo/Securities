#!/usr/bin/env python3
"""EQUS.MINI ``db.Live()``: ``SymbolMappingMsg`` (``stype_in_symbol`` → ``stype_out_symbol``, ``instrument_id``). Key: ``conf/config.ini`` ``api_key`` (or ``DATABENTO_API_KEY``).

One ticker (positional): subscribe with ``stype_in=raw_symbol``, stop on first mapping.

No positional: load tickers from ``XNAS-XCBOE-underlyings.csv`` (v4 root), ``stype_in=raw_symbol``. Override list via ``--symbols-file``. Each symbol gets its own Live session; rows append to ``YYYYMMDD/raw/XNAS-DATABENTO.csv``.
"""
...
DATASET = "EQUS.MINI"
DEFAULT_UNDERLYINGS_CSV = Path(__file__).resolve().parent / "XNAS-XCBOE-underlyings.csv"
...
def _default_csv_out() -> Path:
    from symbology_paths import XNAS_CSV, raw_csv

    return raw_csv(XNAS_CSV)
...
def main() -> int:
    p = argparse.ArgumentParser(
        description=f"{DATASET} Live SymbolMappingMsg (default: XNAS-XCBOE-underlyings.csv, stype_in=raw_symbol; or one ticker positional).",
    )
    p.add_argument(
        "raw",
        nargs="?",
        default=None,
        help='single equity ticker, e.g. "AAPL" (default stype_in=raw_symbol)',
    )
    p.add_argument(
        "--symbols-file",
        default=str(DEFAULT_UNDERLYINGS_CSV),
        help="underlyings list (one ticker per line; default: XNAS-XCBOE-underlyings.csv in v4/)",
    )
    ...
    p.add_argument(
        "--stype-in",
        default="raw_symbol",
        help="Live subscribe stype_in (default: raw_symbol)",
    )
    ...
    p.add_argument(
        ...
        metavar="PATH",
        help="append symbol_mappings (default: YYYYMMDD/raw/XNAS-DATABENTO.csv; empty to skip)",
    )
    p.add_argument(
        "--historical-fallback",
        action="store_true",
    ...
    if one_occ:
        ...
        symbols = [_norm_sym(raw_in).split()[0] if _norm_sym(raw_in) else occ]
        sym_path = None
    else:
    ...
    st_in = (args.stype_in or "").strip() or "raw_symbol"
    ...
    if one_occ:
        ...
        print(f"  subscribe symbol (raw_symbol): {symbols[0]!r}", file=sys.stderr, flush=True)
        t.start()
        try:
            try:
                client.start()
        ...
        try:
            for idx, sym in enumerate(symbols):
                print(
                    f"[{idx + 1}/{len(symbols)}] {DATASET} Live  symbol={sym!r}  "
                ...
                if not parent_live_ok:
                    live_ok = False
                    print(f"error: Live: failed for {sym!r}", file=sys.stderr, flush=True)
            ...
            print("\ninterrupted: stopping symbol loop", file=sys.stderr, flush=True)
            return 130
    ...
    if one_occ:
        ...
        if hit is None:
            hint = (
                f"no mapping for ticker {occ!r} (subscribe tried {symbols[0]!r}). "
