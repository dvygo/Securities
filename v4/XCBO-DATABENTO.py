#!/usr/bin/env python3
"""OPRA.PILLAR ``db.Live()``: ``SymbolMappingMsg`` (``stype_in_symbol`` → ``stype_out_symbol``, ``instrument_id``). Key: ``conf/config.ini`` ``api_key`` (or ``DATABENTO_API_KEY``).
...
No positional: load tickers from ``XNAS-XCBOE-underlyings.csv`` (v4 root), append ``.OPT``, subscribe with ``stype_in=parent``. Override list via ``--symbols-file``. Each parent gets its own Live session; rows append to ``YYYYMMDD/raw/XCBO-DATABENTO.csv``.
"""
...
def _default_csv_out() -> Path:
    from symbology_paths import XCBO_CSV, raw_csv

    return raw_csv(XCBO_CSV)
...
def main() -> int:
    p = argparse.ArgumentParser(
        description=f"{DATASET} Live SymbolMappingMsg (default: XNAS-XCBOE-underlyings.csv → ROOT.OPT, stype_in=parent; or one OCC positional).",
    )
    p.add_argument(
        "raw",
        nargs="?",
    ...
    p.add_argument(
        ...
        help="underlyings list (one ticker per line); bare roots get .OPT (default: XNAS-XCBOE-underlyings.csv in v4/)",
    )
    ...
    p.add_argument(
        ...
        metavar="PATH",
        help="append symbol_mappings (default: YYYYMMDD/raw/XCBO-DATABENTO.csv; empty to skip)",
    )
    p.add_argument(
        "--historical-fallback",
        action="store_true",
