...
```
python -m strategies --strategy=str04test
```

Dispatches to `loader.py:main()` via [strategies/\_\_main\_\_.py](../__main__.py).
Running `loader.py` directly also works: `python -m strategies.STR04TEST.loader`.
...
Dataset comes from `premarket.sources.databento_src.VENUE_CONFIGS` (shared
with the rest of premarket, not duplicated here). Keys come from
`premarket.config.load_databento().keys[EXCHANGE]` -- `conf/keys.ini`
